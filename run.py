...
def run_with_cli_args(args):
    ...
    from CHWOS.SIMILE.train import run, CV, nested_CV, run_from_save
    ...
    if args.create_sweep_param_file != '':
        ...
        if not (args.sweep_output_path != '' and args.sweep_param_path != ''):
            ...
            logger.info(f'Starting sweep with param path {args.sweep_param_path} and output path {args.sweep_output_path}')
            swp.run()
