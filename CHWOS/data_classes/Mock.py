...
class Mock_dataset(BASE_dataset):
    ...
    def load_feature_data(self):
        ...
        self.features[self.A_TAG], self.meta[self.A_TAG] = np.load(
            self.exp_config.DATALOCS[self.exp_config.A_TAG], allow_pickle=True
        )
        self.features[self.B_TAG], self.meta[self.B_TAG] = np.load(
            self.exp_config.DATALOCS[self.exp_config.B_TAG], allow_pickle=True
        )
    ...
    def set_reporting_variables(self):
        ...
        self.removed_classes_count["train"] = {
            self.A_TAG: {**blobclass_dict},
            self.B_TAG: {**blobclass_dict},
        }
        self.removed_classes_count["valid"] = {
            self.A_TAG: {**blobclass_dict},
            self.B_TAG: {**blobclass_dict},
        }
        ...
        self.class_count = {
            "valid": {self.A_TAG: defaultdict(int), self.B_TAG: defaultdict(int)},
            "train": {self.A_TAG: defaultdict(int), self.B_TAG: defaultdict(int)},
        }
        ...
        for classstr in list(self.removed_classes_count["train"].keys()):
            for blobclass in list(self.removed_classes_count["train"][classstr].keys()):
                self.removed_classes_count["train"][classstr][f"Total:{blobclass}"] = self.class_count["train"][
                    classstr
                ][blobclass]
        for classstr in list(self.removed_classes_count["valid"].keys()):
            for blobclass in list(self.removed_classes_count["valid"][classstr].keys()):
                self.removed_classes_count["valid"][classstr][f"Total:{blobclass}"] = self.class_count["valid"][
                    classstr
                ][blobclass]
        ...
        for lbl, lbl2 in [(lbl, lbl2), (lbl2, lbl)]:
            for blobclass in self.lbl_to_pos_neg[lbl]["all"]:
                self.lbl_to_pos_neg[lbl]["neg" if blobclass in self.lbl_to_pos_neg[lbl2]["all"] else "pos"].append(
                    blobclass
                )
    ...
    def calculate_performance_metrics(self, print_perf=True):
        ...
        for lbl in self.removed_classes_count[splittag]:
            ...
            pos_classes = [i for i in self.lbl_to_pos_neg[lbl]["pos"] if isinstance(i, int)]
            ...
            neg_classes = [i for i in self.lbl_to_pos_neg[lbl]["neg"] if isinstance(i, int)]
