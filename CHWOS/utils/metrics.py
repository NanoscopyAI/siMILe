...
def get_accuracy(pos_removed: np.array, total_P: np.array, neg_removed: np.array, total_N: np.array):
    ...
    return_dict = {'P_acc': P_acc, 'N_acc': N_acc, 'PN_acc': PN_acc, 'PN_weighted_acc': PN_weighted_acc}
    if len(total_P) + len(total_N) > 2:
        ...
        return_class_dict = {'class_acc': class_acc, 'class_weighted_acc': class_weighted_acc}
