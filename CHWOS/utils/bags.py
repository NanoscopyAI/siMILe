...
def generate_train_test_validation_bags(features, A_TAG, B_TAG, split, bagsize, mil_labels, seed=None):
    ...
    pos_split_idx = [
        int(len(pos_array) * split[0]),
        int(len(pos_array) * (split[0] + split[1])),
    ]
    neg_split_idx = [
        int(len(neg_array) * split[0]),
        int(len(neg_array) * (split[0] + split[1])),
    ]
...
def get_split_bags_and_labels(featuresSplit, exp_config, gave_splits=False):
    ...
    if gave_splits:  # Used with folds
        ...
        valid_bags, valid_bag_labels = generate_train_test_validation_bags(
            featuresSplit["valid"],
            A_TAG=A_TAG,
            B_TAG=B_TAG,
            split=[1, 0, 0],
        ...
        (
            ...
            valid_bags,
            valid_bag_labels,
        ) = generate_train_test_validation_bags(
            featuresSplit["train"],
            A_TAG=A_TAG,
            B_TAG=B_TAG,
            split=[
                exp_config.train_split,
                exp_config.test_split,
                exp_config.valid_split,
            ],
            bagsize=exp_config.bagsize,
            mil_labels=mil_labels,
        )
