...
from CHWOS.SIMILE.inference import compute_all_accuracy, predict_bag, predict_instances
...
class SimileIterModel:
    ...
    def set_dataset(self, dataset):
        ...
        self.compute_all_accuracy_partial = partial(
            compute_all_accuracy,
            A_TAG=dataset.A_TAG,
            B_TAG=dataset.B_TAG,
            mil_labels=self.exp_config.mil_labels,
        )
    ...
    def train(self):
        ...
        self.trainer = train(
            sigma2=self.exp_config.sigma,
            train_bags=self.dataset.train_bags,
            train_bag_labels=self.dataset.train_bag_labels,
            bagsize=self.exp_config.bagsize,
            C=self.exp_config.C,
        )
    ...
    def predict_bags(self, train=True, valid=True, test=False):
        ...
        splitbags = [
            self.dataset.train_bags,
            self.dataset.valid_bags,
            self.dataset.test_bags,
        ]
        splitlbls = [
            self.dataset.train_bag_labels,
            self.dataset.valid_bag_labels,
            self.dataset.test_bag_labels,
        ]
        results = {}

        for split_str, split_flag, bags, lbls in zip(
            ["train", "valid", "test"], [train, valid, test], splitbags, splitlbls
        ):
            if not split_flag:
                continue
    ...
    def classify_and_cut(self, train=True, valid=True):
        ...
        for split_str, split_flag in zip(["train", "valid"], [train, valid]):
            ...
            was_cut, cutoffs = classify_and_cut(
                prediction_dict=self.ip[split_str],
