...
def classify_symc(dataset, splittag, val_prediction_dict, cutoffs=None): 
    ...
    if not cutoffs:
        kmeans_centers = sorted(KMeans(n_clusters=3, init='random', random_state=dataset.exp_config.seed, n_init=1000).fit(agg_pred_vals.reshape(-1,1)).cluster_centers_)
...
def cut(dataset, splittag, cut_sym=True): 
    ...
    dataset.featuresSplit[splittag][classify_tag] = np.array([i for i in dataset.featuresSplit[splittag][classify_tag] if tuple(i) not in dataset.classified_instances[splittag]])

    if cut_sym:
        logger.debug('Pre cut {} feature shape: {}: {}'.format(splittag, B_TAG, dataset.featuresSplit[splittag][B_TAG].shape))
        dataset.featuresSplit[splittag][B_TAG] = np.array([i for i in dataset.featuresSplit[splittag][B_TAG] if tuple(i) not in dataset.classified_instances[splittag]])
        logger.debug('Post cut {}: New feature shape: {}: {} {}: {}'.format(splittag, A_TAG, dataset.featuresSplit[splittag][A_TAG].shape, B_TAG, dataset.featuresSplit[splittag][B_TAG].shape))
    else:
    ...
    bags, labels = generate_train_test_validation_bags(dataset.featuresSplit[splittag], A_TAG=A_TAG, B_TAG=B_TAG, split=[1,0,0], bagsize=dataset.exp_config.bagsize, mil_labels=dataset.exp_config.mil_labels)
