...
from CHWOS.mil.bag_representation.bag_representation import BagRepresentation
from CHWOS.mil.utils.utils import bags2instances
...
class MILESBase(BagRepresentation):
    ...
    def transform(self, X, return_argmin=False):
        ...
        if not return_argmin:
            ...
            #dist should now contain vector for every bag with the closest distance to each other instance in iip
            return self.similarity_measure(dist)
        else:
            dist = []
            indx = []
            for bag in X:
                out = self.get_bag_instances_distance(bag, return_argmin)
                dist.append(out[0]) #each row is a new bag, each column is an instance with its closest values
                indx.append(out[1]) #same as above but with the idx of the instance in the bag giving the closest value
            
            return np.array(self.similarity_measure(dist)), indx
    def similarity_measure(self, dist):
        ...
        """
        return np.exp(-np.double(np.array(dist))**2./self.sigma2)
    ...
    def get_bag_instances_distance(self, bag, return_argmin=False):
        ...
        """
        d = np.array([self.get_instance_instances_distance(np.array(ins, dtype=np.float64)) for ins in bag])

        if return_argmin:
            return np.min(d, axis=0), np.where(d == np.min(d, axis=0)) #np.argmin(d, axis=0)
            #np.where will return a 2d tuple of 2 arrays, the first contains the row indices, the second the column indices, includes repeats
        else:
