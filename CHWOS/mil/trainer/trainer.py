...
from CHWOS.mil.metrics.manager import MetricsManager
from CHWOS.mil.errors.custom_exceptions import DimensionError, FitNonCalledError, PrepareNonCalledError
from CHWOS.mil.utils.utils import recursive_len, get_samples_weight
from CHWOS.mil.validators import PredefinedSplit
...
class Trainer:
    ...
    #should be combined with above method, but I only use MILEs and don't want issues with his code

    def predict_instances(self, bag, use_threshold=False):
        if len(self.pipeline) > 1:
            bag = self.pipeline[:-1].transform(bag)
        return self.model.predict_instances(bag, use_threshold=use_threshold)